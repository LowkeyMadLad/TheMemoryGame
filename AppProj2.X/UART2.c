...
{	
   InitUART2();	//Can also Initialize UART2 module once at start of main() 
...
void Disp2Input(unsigned int DispNum)
{
    char rem[1] = {0}; // array to store remainders
    char rem1;
    // uint16_t quot = 0;  // quotient
    uint8_t i=0; 
    for(i = 0; i<1; i++)
...
void DispDiff(int diff){
    XmitUART2(32,50); // blank a line
    XmitUART2(13,1);  // carriage return
    Disp2String("Selected Difficulty: ");
    switch(diff){
        case(1): 
            Disp2String("Very Easy");
            break;
        case(2):
            Disp2String("Easy");
            break;
        ...
        case(4):
            ...
            break;
        case(5):
            Disp2String("Very Hard");
            break;
        default:
            Disp2String("None Selected");
            break;
    }
    XmitUART2(13, 1); // Carriage return character
